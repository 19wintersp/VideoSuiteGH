...
namespace StitcherNC
{
    public partial class Needle : Form
    {
        ...
        private ContainerControl refbox, refboxparent, framelistparent;
        private FlowLayoutPanel framelist;
        ...
        */
        ...
        private void InitializeUi(bool showLauncher)
        {
            ...
            filemenu.DropDownItems.Add(new ToolStripSeparator());
            filemenu.DropDownItems.Add("Configuration", (Image)null);
            filemenu.DropDownItems.Add(new ToolStripSeparator());
            ...
            videomenu.DropDownItems.Add("New Frame", (Image)null, (object sender, EventArgs e) => { AddFrame(@"C:\Users\Paddy\Documents\Is this a memory leak.png"); }); //FIX
            ...
            /*videoPlayer = new WMPLib.WindowsMediaPlayer() {
                URL = @"C:\Users\Paddy\Downloads\Chrome Downloads\gomezgel.mp4",
                uiMode = "none"
            };
            ...
            if (showLauncher)
            {
                launcher = new Form() { Width = 182, Height = 123, Text = "Start", ControlBox = false, ShowIcon = false,/* Icon = new Icon("img/stitcher.ico")*/ };
        ...
        private void HorizontalScrollBars(ScrollableControl ctrl) { ScrollBars(ctrl, ctrl.VerticalScroll); }
        ...
        private void UpdateEdit()
        {
            ...
            foreach (List<string> scene in film)
            {
                ...
                {
                    //FRAME
                    if (film.IndexOf(scene) == openScene)
                    {
                        //CURRENT
                        framelist.Controls.Add(new FrameTile(frame, (bool dir) => {
                            int frameIndex = scene.IndexOf(frame);
                            ...
                            tile.DestroyTile();
                            scene.RemoveAt(scene.IndexOf(frame));
                            DrawUi();
                        }, (FrameTile tile) => {
                            //EDIT FRAME
        ...
        {
            //Select frame from referenced images and call AddFrame
        }
        private void AddFrame(string path) { if (film.Count == 0) return; AddFrame(path, 0); }
        ...
        /*async */private /*Task*/void CameraFrame()
        {
            if (meta == null)
            {
                MessageBox.Show("No open document.", "Error", MessageBoxButtons.OK, MessageBoxIcon.Warning);
    ...
    {
        public int stdWidth = 160, stdHeight = 120;
        ...
        public FrameTile(string imagePath, Action<bool> moveHandler, Action<FrameTile> deleteHandler, Action<FrameTile> selectionHandler)
        {
            ...
            moveButts.Item1.Click += (object sender, EventArgs e) => { moveHandler(false); };
            moveButts.Item2.Click += (object sender, EventArgs e) => { moveHandler(true); };
    ...
    public class SavedDocument
    {
        ...
        public Tuple<Dictionary<string, string>, List<List<string>>> ToDoc()
        {
            ...
            for (int i = 0; i < scenes.Length / 3; i++) scs.Add(int.Parse(scenes[i].ToString() + scenes[i + 1].ToString() + scenes[i + 2].ToString()));
